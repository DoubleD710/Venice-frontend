...
export function runMemoryRuntimeTests() {
  ...
  const transitioned = runtime.transitionCard('card-1', MEMORY_LIFECYCLE_STATES.archived);
  ...
  return [
    ...
    assert('lifecycle transition', transitioned.card.lifecycleState === MEMORY_LIFECYCLE_STATES.archived),
    assert('delete behavior', deleted.card.lifecycleState === MEMORY_LIFECYCLE_STATES.deleted),
    assert('merge behavior', merged.status === 'complete' && merged.card.tags.includes('secondary') && merged.mergedCard.lifecycleState === MEMORY_LIFECYCLE_STATES.merged),
